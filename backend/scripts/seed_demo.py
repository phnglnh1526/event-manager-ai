...
Run manually. This module is never imported by application startup and never
changes schema. It only reconciles records identified by the fixed demo emails
and exact demo Event titles below.
"""
...
USERS = (
    ...
    *tuple((f"attendee-{i:02d}-demo@example.com", f"Attendee {i:02d} Demo", "ATTENDEE", True) for i in range(1, 9)),
    ("inactive-demo@example.com", "Inactive Demo", "ATTENDEE", False),
)

EVENTS = (
    (MAIN_EVENT_TITLE, "organizer-a-demo@example.com", "A complete demonstration event used to test registration, tickets, QR check-in, feedback, analytics and AI features.", "ICTU - Main Hall", datetime(2026, 10, 10, 8), datetime(2026, 10, 10, 11), "PUBLISHED", 100),
    ("AI Technology Conference 2026", "organizer-a-demo@example.com", "Hội thảo giới thiệu các xu hướng trí tuệ nhân tạo, ứng dụng AI trong phát triển phần mềm và các bài toán thực tế.", "ICTU - Tòa nhà A", datetime(2026, 10, 11, 13, 30), datetime(2026, 10, 11, 17, 30), "PUBLISHED", 200),
    ("Web Development Workshop 2026", "organizer-a-demo@example.com", "DEMO dataset: draft Event for registration and visibility tests.", "ICTU - Web Lab", datetime(2026, 10, 12, 8), datetime(2026, 10, 12, 12), "DRAFT", 80),
    ("Cloud Computing Seminar", "organizer-a-demo@example.com", "DEMO dataset: cancelled Event for lifecycle and error tests.", "ICTU - Cloud Lab", datetime(2026, 10, 13, 14), datetime(2026, 10, 13, 18), "CANCELLED", 100),
    ("Cybersecurity Conference 2026", "organizer-b-demo@example.com", "DEMO dataset: published Event owned by Organizer B.", "ICTU - Security Hall", datetime(2026, 10, 14, 8, 30), datetime(2026, 10, 14, 16, 30), "PUBLISHED", 120),
)
...
def upsert_users(db, password):
    ...
    for email, full_name, role, is_active in USERS:
        ...
        if user is None:
            ...
            db.add(user)
            db.flush()
        user.full_name, user.role, user.is_active = full_name, role, is_active
...
def upsert_events(db, users):
    ...
    for title, owner_email, description, location, start, end, status, capacity in EVENTS:
        ...
        if event is None:
            ...
            current_owner_email = db.scalar(select(User.email).where(User.id == event.owner_id)) or ""
            is_demo_owned = current_owner_email.endswith("-demo@example.com")
            is_legacy_main = title == MAIN_EVENT_TITLE and current_owner_email.endswith(LEGACY_DEMO_SUFFIX)
...
def reconcile_speakers(db, event, specs):
    ...
    for full_name, title, organization, bio in specs:
        speaker = one_or_none(db, Speaker, Speaker.event_id == event.id, Speaker.full_name == full_name)
        if speaker is None:
            speaker = Speaker(event_id=event.id, full_name=full_name)
            db.add(speaker)
            db.flush()
        speaker.title, speaker.organization, speaker.bio, speaker.email = title, organization, bio, None
        result[full_name] = speaker
    extras = list(db.scalars(select(Speaker).where(Speaker.event_id == event.id, Speaker.full_name.not_in(desired))).all())
    for extra in extras:
        db.execute(delete(Schedule).where(Schedule.event_id == event.id, Schedule.speaker_id == extra.id))
        db.delete(extra)
    return result
...
def reconcile_schedules(db, event, speakers, specs):
    ...
    for title, sh, sm, eh, em, speaker_name, location in specs:
        schedule = one_or_none(db, Schedule, Schedule.event_id == event.id, Schedule.title == title)
        if schedule is None:
    ...
    db.execute(delete(Schedule).where(Schedule.event_id == event.id, Schedule.title.not_in(desired)))


def remove_legacy_main_rows(db, main_event, target_user_ids):
    for feedback in list(db.scalars(select(Feedback).where(Feedback.event_id == main_event.id)).all()):
        if feedback.user_id not in target_user_ids:
            email = db.scalar(select(User.email).where(User.id == feedback.user_id)) or ""
            if not email.endswith(LEGACY_DEMO_SUFFIX):
                raise RuntimeError("Main demo Event contains non-demo Feedback; refusing cleanup")
            db.delete(feedback)
    for registration in list(db.scalars(select(Registration).where(Registration.event_id == main_event.id)).all()):
        if registration.user_id not in target_user_ids:
            email = db.scalar(select(User.email).where(User.id == registration.user_id)) or ""
            if not email.endswith(LEGACY_DEMO_SUFFIX):
                raise RuntimeError("Main demo Event contains non-demo Registration; refusing cleanup")
            # Use the database FK cascade. ORM relationship assignment would try
            # to set the non-null Ticket.registration_id to NULL first.
            db.execute(delete(Registration).where(Registration.id == registration.id))
    db.flush()


def ensure_registration(db, event, attendee, status):
    registration = one_or_none(db, Registration, Registration.event_id == event.id, Registration.user_id == attendee.id)
    if registration is None:
        registration = Registration(event_id=event.id, user_id=attendee.id, status=status)
        db.add(registration)
        db.flush()
    ticket = one_or_none(db, Ticket, Ticket.registration_id == registration.id)
    if ticket is None:
        ticket = create_ticket_with_retry(db, registration.id, "ACTIVE" if status == "REGISTERED" else "VOID")
        db.flush()
...
def reconcile_main_lifecycle(db, main_event, users):
    ...
    for index, attendee in enumerate(attendees, start=1):
        _, tickets[index] = ensure_registration(db, main_event, attendee, "CANCELLED" if index == 8 else "REGISTERED")
    ...
    for index, rating, comment in FEEDBACKS:
        ...
        feedback = one_or_none(db, Feedback, Feedback.event_id == main_event.id, Feedback.user_id == attendee.id)
        if feedback is None:
            feedback = Feedback(event_id=main_event.id, user_id=attendee.id)
            db.add(feedback)
        feedback.rating, feedback.comment = rating, comment
    db.execute(delete(Feedback).where(Feedback.event_id == main_event.id, Feedback.user_id.not_in(desired_feedback_users)))
...
def reconcile_announcements(db, event, creator):
    ...
    for title, content, status in ANNOUNCEMENTS:
        announcement = one_or_none(db, Announcement, Announcement.event_id == event.id, Announcement.title == title)
    ...
    db.execute(delete(Announcement).where(Announcement.event_id == event.id, Announcement.title.not_in(desired)))


def verify_dataset(db, main, users):
    tables = sorted(inspect(engine).get_table_names())
    expected_tables = sorted(("users", "events", "speakers", "schedules", "registrations", "tickets", "checkins", "feedbacks", "announcements"))
    if tables != expected_tables:
        raise RuntimeError(f"Unexpected schema: {tables}")
    counts = {
        "speakers": db.scalar(select(func.count(Speaker.id)).where(Speaker.event_id == main.id)),
        "schedules": db.scalar(select(func.count(Schedule.id)).where(Schedule.event_id == main.id)),
        "registrations": db.scalar(select(func.count(Registration.id)).where(Registration.event_id == main.id)),
        "registered": db.scalar(select(func.count(Registration.id)).where(Registration.event_id == main.id, Registration.status == "REGISTERED")),
        "cancelled": db.scalar(select(func.count(Registration.id)).where(Registration.event_id == main.id, Registration.status == "CANCELLED")),
        "checkins": db.scalar(select(func.count(CheckIn.id)).join(Ticket).join(Registration).where(Registration.event_id == main.id)),
        "feedbacks": db.scalar(select(func.count(Feedback.id)).where(Feedback.event_id == main.id)),
        "announcements": db.scalar(select(func.count(Announcement.id)).where(Announcement.event_id == main.id)),
    }
    expected = {"speakers": 4, "schedules": 6, "registrations": 8, "registered": 7, "cancelled": 1, "checkins": 6, "feedbacks": 5, "announcements": 4}
    if counts != expected:
        raise RuntimeError(f"Demo invariant mismatch: {counts}")
    ratings = Counter(db.scalars(select(Feedback.rating).where(Feedback.event_id == main.id)).all())
    average = float(db.scalar(select(func.avg(Feedback.rating)).where(Feedback.event_id == main.id)) or 0)
    if ratings != Counter({5: 2, 4: 2, 3: 1}) or average != 4.2:
        raise RuntimeError(f"Feedback metrics mismatch: ratings={ratings}, average={average}")
    ...
    failures = {name: db.scalar(text(query)) for name, query in integrity_queries.items()}
...
def run():
    ...
    before_tables = sorted(inspect(engine).get_table_names())
    db = SessionLocal()
    try:
        ...
        if before_tables != after_tables:
            raise RuntimeError("Schema changed while seeding demo data")
        db.commit()
        print("COMPLETE_DEMO_DATA_READY")
        print(f"TABLE_COUNT={len(after_tables)}")
        print(f"MAIN_EVENT_ID={main.id}")
