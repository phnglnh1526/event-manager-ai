...
function StaffCheckInPage({ token, currentUser, onLogout, onUnauthorized }) {
  const [events, setEvents] = useState([]), [selectedId, setSelectedId] = useState(""), [ticketCode, setTicketCode] = useState("");
  const [loading, setLoading] = useState(true), [submitting, setSubmitting] = useState(false), [pageError, setPageError] = useState(""), [result, setResult] = useState(null);
  const [scannerOpen, setScannerOpen] = useState(false);
  const inputRef = useRef(null), inFlight = useRef(false);
  ...
  const performCheckIn = async (candidate) => {
    ...
    const code = candidate.trim(), event = selectedEvent;
    ...
    try {
      const data = await checkInTicket(event.id, code, token);
      setTicketCode(""); setResult({ type: "success", data });
    } catch (error) {
      if (error.status === 401) onUnauthorized();
      else setResult({ type: "error", message: mapError(error) });
    } finally {
      inFlight.current = false; setSubmitting(false); setTimeout(() => inputRef.current?.focus(), 0);
    }
  };
