...
import { createEvent, deleteEvent, getEvent, getEvents, updateEvent, uploadEventImage, getAssetUrl } from "../services/api";
...
function EventsPage({ token, currentUser, onLogout, onUnauthorized, activeView, onViewChange }) {
  ...
  const resetEditor = () => { setEditorId(undefined); setForm(EMPTY_FORM); setImageFile(null); setDirty(false); setFormError(""); };
  ...
  const changeImage = (file) => {
    if (file && file.size > 5 * 1024 * 1024) {
      setFormError("Event image must be 5 MB or smaller.");
      setImageFile(null);
      return;
    }
    setImageFile(file);
  ...
  const loadEvent = async (eventId, editMode, targetTab = "overview") => {
    ...
    try {
      ...
      if (editMode) {
        ...
        setForm({ title: event.title, description: event.description || "", location: event.location, start_time: toDateTimeLocalValue(event.start_time), end_time: toDateTimeLocalValue(event.end_time), status: event.status, max_attendees: String(event.max_attendees) }); setImageFile(null);
  ...
  const submit = async () => {
    ...
    try {
      let result = editorId == null ? await createEvent(payload, token) : await updateEvent(editorId, payload, token);
      if (imageFile) {
        result = await uploadEventImage(result.id, imageFile, token);
      }
