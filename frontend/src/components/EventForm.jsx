    ...
    <section className="event-editor" aria-labelledby="event-form-title">
      <div className="editor-heading">
        <div>
          ...
          <h2 id="event-form-title">{isEditing ? "Update event" : "Create event"}</h2>
        </div>
        <button
          type="button"
      ...
      <EventImageUpload
        ...
        onFileSelect={(file) => onChange("pendingFile", file)}
        onRemove={() => {
          onChange("pendingFile", null);
          onChange("cover_image_url", null);
        }}
        uploading={loading && Boolean(form.pendingFile)}
        disabled={loading}
      />
      ...
      <div className="event-options-grid">
        ...
        <div className="editor-field">
          <label htmlFor="event-capacity">Maximum attendees *</label>
          <input
            id="event-capacity"
            type="number"
            min="1"
